...
; ule 0 is equivalent to eq 0 when elements are non-negative.
define i1 @reduce_add_zext_v4i1_ule(<4 x i32> %a, <4 x i32> %b) {
; CHECK-LABEL: define i1 @reduce_add_zext_v4i1_ule(
; CHECK-SAME: <4 x i32> [[A:%.*]], <4 x i32> [[B:%.*]]) {
; CHECK-NEXT:    [[CMP:%.*]] = icmp eq <4 x i32> [[A]], [[B]]
; CHECK-NEXT:    [[TMP1:%.*]] = call i1 @llvm.vector.reduce.umax.v4i1(<4 x i1> [[CMP]])
; CHECK-NEXT:    [[RES:%.*]] = icmp eq i1 [[TMP1]], false
...
;
  ...
  %zext = zext <4 x i1> %cmp to <4 x i32>
  %red = call i32 @llvm.vector.reduce.add.v4i32(<4 x i32> %zext)
  %res = icmp ule i32 %red, 0
  ret i1 %res
}

; ugt 0 is equivalent to ne 0 when elements are non-negative.
define i1 @reduce_add_zext_v4i1_ugt(<4 x i32> %a, <4 x i32> %b) {
; CHECK-LABEL: define i1 @reduce_add_zext_v4i1_ugt(
; CHECK-SAME: <4 x i32> [[A:%.*]], <4 x i32> [[B:%.*]]) {
; CHECK-NEXT:    [[CMP:%.*]] = icmp eq <4 x i32> [[A]], [[B]]
; CHECK-NEXT:    [[RES:%.*]] = call i1 @llvm.vector.reduce.umax.v4i1(<4 x i1> [[CMP]])
; CHECK-NEXT:    ret i1 [[RES]]
;
  ...
  %zext = zext <4 x i1> %cmp to <4 x i32>
  %red = call i32 @llvm.vector.reduce.add.v4i32(<4 x i32> %zext)
  %res = icmp ugt i32 %red, 0
  ret i1 %res
}

; ule 0 with non-positive elements (sext i1): still folds to reduce.umax==0.
define i1 @reduce_add_sext_v4i1_ule(<4 x i32> %a, <4 x i32> %b) {
; CHECK-LABEL: define i1 @reduce_add_sext_v4i1_ule(
; CHECK-SAME: <4 x i32> [[A:%.*]], <4 x i32> [[B:%.*]]) {
; CHECK-NEXT:    [[CMP:%.*]] = icmp eq <4 x i32> [[A]], [[B]]
; CHECK-NEXT:    [[TMP1:%.*]] = call i1 @llvm.vector.reduce.umax.v4i1(<4 x i1> [[CMP]])
; CHECK-NEXT:    [[RES:%.*]] = icmp eq i1 [[TMP1]], false
