...
bool VectorCombine::foldReduceAddCmpZero(Instruction &I) {
  ...
  // Use the recursive constant range so that e.g. sext i1 (range {-1, 0}) is
  // detected as non-positive and ranged add/sub chains get tightened bounds.
  SimplifyQuery Q = SQ.getWithInstruction(&I);
  ConstantRange CR = computeConstantRangeRecursive(Vec, /*ForSigned=*/true, Q);
  if (CR.isEmptySet())
    return false;
  bool IsNonNegative = CR.getSignedMin().isNonNegative();
  bool IsNonPositive = CR.getSignedMax().isNonPositive();
  if (!IsNonNegative && !IsNonPositive)
    return false;
  unsigned NumSignBits = IsNonNegative ? CR.getUnsignedMax().countLeadingZeros()
                                       : CR.getSignedMin().countLeadingOnes();
  ...
  // For signed predicates, need a tighter bound to prevent a sign flip.
  if (!IsNonNegative &&
